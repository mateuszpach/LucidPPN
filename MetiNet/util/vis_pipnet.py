...
def visualize_topk(net, projectloader, device, foldername, args: argparse.Namespace, k=10):
    ...
    for i, (xs, ys) in img_iter:
        ...
        with torch.no_grad():
            ...
            grouped_proto_features, grouped_pooled, out = net(xs, xs, args.use_classification_layer)
            bs, _, _, fs, fs = grouped_proto_features.shape
            pfs = torch.reshape(grouped_proto_features, (bs, -1, fs, fs))
            pooled = torch.reshape(grouped_pooled, (bs, -1))
    ...
    for p in topks.keys():
        ...
        for idx, score in topks[p]:
            alli.append(idx)
            if score > 0.1 or True:  #in case prototypes have fewer than k well-related patches
                found = True
        if not found:
