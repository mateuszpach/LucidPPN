...
def get_data(args: argparse.Namespace): 
    ...
    if args.dataset =='CUB-200-2011':     
        return get_birds(True, '/shared/sets/datasets/lucid/cub/CUB_200_2011/dataset/train_crop', '/shared/sets/datasets/lucid/cub/CUB_200_2011/dataset/train', '/shared/sets/datasets/lucid/cub/CUB_200_2011/dataset/test_crop', args.image_size, args.seed, args.validation_size, '/shared/sets/datasets/lucid/cub/CUB_200_2011/dataset/train', '/shared/sets/datasets/lucid/cub/CUB_200_2011/dataset/test_full', option=args.option)
    ...
    if args.dataset == 'CARS':
        return get_cars(True, './shared/sets/datasets/lucid/cars/train', '/shared/sets/datasets/lucid/cars/train', '/shared/sets/datasets/lucid/cars/test', args.image_size, args.seed, args.validation_size)
    if args.dataset == 'FLOWERS':
        return get_flowers(True, '/shared/sets/datasets/lucid/flowers/train', '/shared/sets/datasets/lucid/flowers/train', '/shared/sets/datasets/lucid/flowers/test', args.image_size, args.seed, args.validation_size)
    if args.dataset == 'DOGS':
        return get_dogs(True, '/shared/sets/datasets/lucid/dogs/train', '/shared/sets/datasets/lucid/dogs/train', '/shared/sets/datasets/lucid/dogs/test', args.image_size, args.seed, args.validation_size)
...
def create_datasets(transform1, transform2, transform_no_augment, num_channels:int, train_dir:str, project_dir: str, test_dir:str, seed:int, validation_size:float, train_dir_pretrain = None, test_dir_projection = None, transform1p=None):
    ...
    projectset = torchvision.datasets.ImageFolder(project_dir, transform=transforms.Compose([
...
def get_birds(augment: bool, train_dir:str, project_dir: str, test_dir:str, img_size: int, seed:int, validation_size:float, train_dir_pretrain = None, test_dir_projection = None, option=0):
    ...
    if option == 0:
        transform_no_augment = transforms.Compose([
            transforms.Resize(size=(img_size, img_size)),
            transforms.RandomResizedCrop(size=(img_size, img_size), scale=(1.0, 1.0), ratio=(1.0, 1.0)),
            transforms.ToTensor(),
            normalize
        ])
    elif option == 1:
        transform_no_augment = transforms.Compose([
            transforms.Resize(size=(img_size, img_size)),
            transforms.RandomResizedCrop(size=(img_size, img_size), scale=(0.75, 0.75), ratio=(1.0, 1.0)),
            transforms.ToTensor(),
            normalize
        ])
    elif option == 2:
        transform_no_augment = transforms.Compose([
            transforms.Resize(size=(img_size, img_size)),
            transforms.RandomResizedCrop(size=(img_size, img_size), scale=(0.5, 0.5), ratio=(1.0, 1.0)),
            transforms.ToTensor(),
            normalize
        ])
    elif option == 3:
        transform_no_augment = transforms.Compose([
            transforms.Resize(size=(img_size, img_size)),
            transforms.RandomResizedCrop(size=(img_size, img_size), scale=(0.25, 0.25), ratio=(1.0, 1.0)),
            transforms.ToTensor(),
            normalize
        ])
    elif option == 4:
        transform_no_augment = transforms.Compose([
            transforms.Resize(size=(img_size, img_size)),
            transforms.Lambda(lambda x: random_hue_jitter(x, 0.5)),
            normalize
        ])
    elif option == 5:
        transform_no_augment = transforms.Compose([
    ...
    if augment:
        ...
        transform2 = transforms.Compose([
                            # transforms.ColorJitter(hue=0.5), # UNCOMMENT TO TEST ON COLOR DISTURBED IMAGES
                            TrivialAugmentWideNoShape(),
                            transforms.RandomCrop(size=(img_size, img_size)), #includes crop
                            transforms.ToTensor(),
                            normalize
...
def get_flowers(augment: bool, train_dir: str, project_dir: str, test_dir: str, img_size: int, seed: int, validation_size: float):
    shape = (3, img_size, img_size)
    mean = (0.485, 0.456, 0.406)
    std = (0.229, 0.224, 0.225)
...
def get_dogs(augment: bool, train_dir: str, project_dir: str, test_dir: str, img_size: int, seed: int, validation_size: float):
    shape = (3, img_size, img_size)
    mean = (0.485, 0.456, 0.406)
    std = (0.229, 0.224, 0.225)
